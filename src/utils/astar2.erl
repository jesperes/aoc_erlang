...
-type nbr_fun() :: fun((search_node()) -> [search_node()]).
-type dist_fun() :: fun((search_node(), search_node()) -> integer()).
-type end_fun() :: fun((search_node()) -> boolean()).
-type astar_result() ::
    {Distance :: integer(), Path :: [search_node()]} | search_exhausted.
...
astar(Start, End, CostFn, NbrFn, DistFn) ->
    case is_function(End) of
        ...
        false ->
            astar0(Start, CostFn, NbrFn, DistFn, fun(E) -> E == End end)
    end.
            ...
            case EndFn(Curr) of
                ...
                false ->
                    ...
                    {_, OC1, CF0, Gs0, Fs1, _, _} =
                        lists:foldl(fun astar_nbr/2,
                                    {Curr, OC0, CF, Gs, Fs0, CostFn, DistFn},
                                    Nbrs),
...
-spec astar_nbr(Nbr :: search_node(), AccIn :: nbr_fold_state()) -> nbr_fold_state().
...
%%% Helper functions
-spec path_recon(Curr :: search_node(), CF :: map()) -> [search_node()].
path_recon(Curr, CF) ->
    case maps:is_key(Curr, CF) of
        true ->
            [Curr | path_recon(maps:get(Curr, CF), CF)];
        false ->
            [Curr]
    end.
