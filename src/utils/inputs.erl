...
-export([get_as_binary/2, get_as_string/2, get_as_lines/2, get_as_ints/2, parse_lines/3]).
...
-spec get_as_lines(Year :: integer(), Day :: integer()) -> [string()].
...
-spec parse_lines(Binary :: binary(), Delims :: string(), Fun :: fun()) -> [term()].
parse_lines(Binary, Delims, Fun) ->
    lists:map(fun(Line) -> Fun(string:tokens(Line, Delims)) end,
...
get_input_dir() ->
    filename:join(
        code:priv_dir(aoc_erlang), "inputs").

-spec get_input_filename(Year :: integer(), Day :: integer()) -> file:filename().
